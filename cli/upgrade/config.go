...
// download & check latest version file
func (u *Config) GetLatestVersion() (v1 semver.Version, err error) {
	var (
		verfile  = "version.txt"
		verurl   = DownloadURL("latest", verfile)
		qverpath = u.fm.Path(u.q, verfile)
	)
	if err := u.Download(qverpath, verurl, u.user, u.pass); err != nil {
		return semver.Version{}, err
	}
	vf, err := os.Open(qverpath)
	if err != nil {
		return semver.Version{}, err
	}
	defer vf.Close()
	b, err := ioutil.ReadAll(vf)
	if err != nil {
		return semver.Version{}, err
	}
	v1, err = semver.Parse(strings.ReplaceAll(string(b), "\n", ""))
	return
}
...
func (u *Config) GetChangelog(ver semver.Version) (_ string, err error) {
	var (
		...
		chgurl   = DownloadURL(ver.String(), path.Join("changelogs", chgfile))
	)
...
func (u *Config) GetHash(ver semver.Version) (sha512 []byte, err error) {
	var (
		...
		hshurl   = DownloadURL(ver.String(), hshfile)
...
func (u *Config) GetBinary(ver, binpath string, s512 []byte) (err error) {
	var (
		binurl = DownloadURL(ver, u.binfile)
	)
...
func (u *Config) Upgrade(ex Executor, v0, v1 semver.Version) (err error) {
	...
	// get & verify binary
	if err = u.GetBinary(v1.String(), qnewbinpath, s512); err != nil {
		return fmt.Errorf(
